...
tell application "System Events"
   tell process "MenubarX"
      ...
      -- perform action "AXPre   ss" of (menu bar item (iconIndex) of m)
      set p to position of menu bar item (iconIndex) of m
      set x to item 1 of p
      set y to item 2 of p
      do shell script cliclick & " r c:" & x & "," & y & " m:" & Storage # AXShowMenu workaround (right click then move mouse to OG location)
   end tell
end tell
