...
tell application tarApp
   try
      ...
      if c is equal to 0 then return "0 windows"
		...
		set collapsed of (first window whose index is closeIndex) to true
		set c to -(c)
   end try
end tell
...
# by telling the process:  minimize active window (and DO NOT cycle next window afterwards (if it's not the next window))
...
tell application "System Events"
	tell process tarAppPName
		...
		# app is not scriptable (couldn't minimize)
		if c < 0 then click (tarWin's buttons whose subrole is "AXMinimizeButton")
	end tell
end tell
