...
$path = 'C:\Users\steven\proj\powershell\add-default-psd-prproj-contextmenu newitems\model_files\Untitled.prproj'

# load binary
$bytes = Get-Content -Path $path -AsByteStream -Raw

# add to regedit, if exists remove or use set-itemproperty
New-ItemProperty -Path registry::HKEY_CLASSES_ROOT\.prproj\ShellNew -PropertyType "Binary" -Name data -Value ([byte[]]($bytes))
